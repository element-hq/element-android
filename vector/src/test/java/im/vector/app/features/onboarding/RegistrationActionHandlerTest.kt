...
class RegistrationActionHandlerTest {
    ...
    fun `when processing SendAgainThreePid, then ignores result`() = runTest {
        ...
        fakeWizardActionDelegate.givenResultsFor(listOf(sendAgainThreePid to RegistrationResult.Success(A_SESSION)))
    ...
    @Test
    fun `when processing AddThreePid, then ignores result`() = runTest {
        val addThreePid = RegisterAction.AddThreePid(RegisterThreePid.Email("an email"))
        fakeWizardActionDelegate.givenResultsFor(listOf(addThreePid to RegistrationResult.Success(A_SESSION)))

        val result = registrationActionHandler.processAction(addThreePid)
    ...
    fun `given wizard delegate returns success, when handling action, then returns success`() = runTest {
        fakeWizardActionDelegate.givenResultsFor(listOf(RegisterAction.StartRegistration to RegistrationResult.Success(A_SESSION)))
    ...
    fun `given password already sent and missing mandatory dummy stage, when handling action, then fast tracks the dummy stage`() = runTest {
        ...
        fakeWizardActionDelegate.givenResultsFor(
                listOf(
                        ...
                        RegisterAction.RegisterDummy to RegistrationResult.Success(A_SESSION)
                )
        )
    ...
    private fun aFlowResult(missingStages: List<Stage>) = RegistrationResult.FlowResponse(
            FlowResult(
    ...
    private fun anUnsupportedResult() = RegistrationResult.FlowResponse(
            FlowResult(
