...
class RegistrationWizardActionDelegateTest {
    ...
    private suspend fun testSuccessfulActionDelegation(case: Case) {
        ...
        val fakeAuthenticationService = FakeAuthenticationService().also { it.givenRegistrationWizard(fakeRegistrationWizard) }
        val registrationWizardActionDelegate = RegistrationWizardActionDelegate(fakeAuthenticationService)

        val result = registrationWizardActionDelegate.executeAction(case.action)
