...
internal class DefaultGetUrlTask @Inject constructor(
        ...
        private val rawCacheStore: RawCacheStore,
) : GetUrlTask {
    ...
    private suspend fun doRequestWithCache(url: String, validityDurationInMillis: Long, strict: Boolean): String {
        // Get data from cache
        val cacheEntity = rawCacheStore.getWithUrl(url)
        val dataFromCache = cacheEntity?.data
        val isCacheValid = cacheEntity != null && Date().time < cacheEntity.lastUpdatedTimestamp + validityDurationInMillis
        ...
        // Store cache
        rawCacheStore.upsertWith(url, data)
        return data
    }
}
